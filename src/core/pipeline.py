...
from src.output_normalizer import normalize_dashboard, should_prefer_ai_stock_name
...
class StockAnalysisPipeline:
    ...
    def _agent_result_to_analysis_result(
        ...
        if agent_result.success and agent_result.dashboard:
            dash = normalize_dashboard(agent_result.dashboard)
            ai_stock_name = str(dash.get("stock_name", "")).strip()
            if ai_stock_name and should_prefer_ai_stock_name(stock_name, ai_stock_name, code):
    ...
    def _describe_volume_ratio(self, volume_ratio: float) -> str:
        ...
        """
        if volume_ratio < 0.5:
            return "Extremely light volume"
        elif volume_ratio < 0.8:
            return "Clearly shrinking volume"
        elif volume_ratio < 1.2:
            return "Normal"
        elif volume_ratio < 2.0:
            return "Moderately expanding volume"
        elif volume_ratio < 3.0:
            return "Clearly expanding volume"
        else:
            return "Very heavy volume"
    ...
    def _compute_ma_status(close: float, ma5: float, ma10: float, ma20: float) -> str:
        ...
        if close > ma5 > ma10 > ma20 > 0:
            ...
            return "Short-term Strengthening 🔼"
        elif close < ma5 and ma5 < ma10:
