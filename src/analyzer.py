...
def apply_placeholder_fill(result: "AnalysisResult", missing_fields: List[str]) -> None:
    ...
    for field in missing_fields:
        if field == "sentiment_score":
            ...
            result.operation_advice = result.operation_advice or "TBD"
        elif field == "analysis_summary":
            result.analysis_summary = result.analysis_summary or "TBD"
            ...
            result.dashboard["core_conclusion"]["one_sentence"] = (
                result.dashboard["core_conclusion"].get("one_sentence") or "TBD"
            )
            ...
            result.dashboard["battle_plan"]["sniper_points"]["stop_loss"] = "TBD"
...
Output the complete Decision Dashboard in JSON format."""
    ...
    def _build_integrity_complement_prompt(self, missing_fields: List[str]) -> str:
        ...
        lines = ["### Completion Requirements: fill the missing required fields below and output the full JSON again:"]
        for f in missing_fields:
            if f == "sentiment_score":
                lines.append("- sentiment_score: 0-100 overall score")
            elif f == "operation_advice":
                lines.append("- operation_advice: Buy/Add/Hold/Reduce/Sell/Wait")
                ...
                lines.append("- dashboard.intelligence.risk_alerts: risk alert list (can be an empty array)")
    ...
    def _build_integrity_retry_prompt(
        ...
        return "\n\n".join([
            base_prompt,
            "### The previous output is below. Complete the missing fields on top of it and output the full JSON again. Do not omit existing fields:",
            previous_output,
            complement,
        ])
    ...
    def _parse_response(
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                # Schema validation (lenient: on failure, continue with normalized dict)
                try:
                    AnalysisReportSchema.model_validate(data)
                except Exception as e:
                    logger.warning(
                        "LLM report schema validation failed, continuing with normalized dict: %s",
                        str(e)[:100],
                    )
                # 提取 dashboard 数据
