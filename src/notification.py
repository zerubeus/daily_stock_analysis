...
class NotificationService(
    ...
    @staticmethod
    def _display_operation_advice(value: str) -> str:
        """Render operation advice in English."""
        normalized = normalize_operation_advice(value)
        return normalized if isinstance(normalized, str) else (value or "")

    @staticmethod
    def _display_trend_prediction(value: str) -> str:
        """Render trend prediction in English."""
    ...
    def generate_daily_report(
        ...
        report_lines = [
            ...
            "",
            f"> Analyzed **{len(results)}** stocks | Generated at: {datetime.now().strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
        ...
        report_lines.extend([
            ...
            "|------|------|",
            f"| 🟢 Buy/Add | **{buy_count}** |",
            f"| 🟡 Hold/Wait | **{hold_count}** |",
            f"| 🔴 Reduce/Sell | **{sell_count}** |",
        ...
        if self._report_summary_only:
            report_lines.extend(["## 📊 Analysis Summary", ""])
            for r in sorted_results:
                emoji = r.get_emoji()
                report_lines.append(
                    f"{emoji} **{r.name}({r.code})**: {self._display_operation_advice(r.operation_advice)} | "
                    f"Score {r.sentiment_score} | {self._display_trend_prediction(r.trend_prediction)}"
                )
        else:
            report_lines.extend(["## 📈 Stock Details", ""])
            # 逐个股票的详细分析
            for result in sorted_results:
                emoji = result.get_emoji()
                confidence_stars = result.get_confidence_stars() if hasattr(result, 'get_confidence_stars') else '⭐⭐'
                display_advice = self._display_operation_advice(result.operation_advice)
                display_trend = self._display_trend_prediction(result.trend_prediction)
                
                report_lines.extend([
                    f"### {emoji} {result.name} ({result.code})",
                    "",
                    f"**Advice**: {display_advice} | **Score**: {result.sentiment_score} | **Trend**: {display_trend} | **Confidence**: {confidence_stars}",
                    "",
                ])
    ...
    def generate_dashboard_report(
        ...
        if results:
            ...
            for r in sorted_results:
                ...
                report_lines.append(
                    f"{signal_emoji} **{display_name}({r.code})**: {self._display_operation_advice(r.operation_advice)} | "
                    f"Score {r.sentiment_score} | {self._display_trend_prediction(r.trend_prediction)}"
                )
            report_lines.extend([
                "",
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                dashboard = normalize_dashboard(result.dashboard) if hasattr(result, 'dashboard') and result.dashboard else {}
                display_trend = self._display_trend_prediction(result.trend_prediction)
                ...
                report_lines.extend([
                    "### 📌 Core Conclusion",
                    "",
                    f"**{signal_emoji} {signal_text}** | {display_trend}",
                    "",
                    f"> **One-line Decision**: {one_sentence}",
                    "",
                ...
                if pos_advice:
                    report_lines.extend([
                        "| Position | Advice |",
                        "|---------|---------|",
                        f"| 🆕 **No Position** | {pos_advice.get('no_position', self._display_operation_advice(result.operation_advice))} |",
                        f"| 💼 **Holding** | {pos_advice.get('has_position', 'Continue holding')} |",
                        "",
                    ])
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        if self._report_summary_only:
            ...
            for r in sorted_results:
                ...
                lines.append(
                    f"{signal_emoji} **{stock_name}({r.code})**: {self._display_operation_advice(r.operation_advice)} | "
                    f"Score {r.sentiment_score} | {self._display_trend_prediction(r.trend_prediction)}"
                )
        else:
            for result in sorted_results:
                ...
                if pos_advice:
                    ...
                    if has_pos:
                        lines.append(f"💼 Holding: {has_pos[:50]}")
                    lines.append("")
        ...
        if models:
            lines.append(f"*Model used: {', '.join(models)}*")
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        lines = [
            f"## 📅 {report_date} Stock Analysis Report",
            "",
            f"> **{len(results)}** stocks | 🟢Buy:{buy_count} 🟡Hold:{hold_count} 🔴Sell:{sell_count} | Avg:{avg_score:.0f}",
            "",
        ]
        ...
        for result in sorted_results:
            ...
            lines.append(f"**{display_advice}** | Score:{result.sentiment_score} | {display_trend}")
        ...
        lines.extend([
            ...
            f"*Full report: reports/report_{report_date.replace('-', '')}.md*"
        ])
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        ...
        lines = [
            f"## {signal_emoji} {stock_name} ({result.code})",
            "",
            f"> {report_date} | Score: **{result.sentiment_score}** | {display_trend}",
            "",
        ]
        ...
        if pos_advice:
            lines.extend([
                "### 💼 Position Advice",
                "",
                f"- 🆕 **No Position**: {pos_advice.get('no_position', display_advice)}",
                f"- 💼 **Holding**: {pos_advice.get('has_position', 'Continue holding')}",
                "",
            ])
        ...
        lines.append("*AI-generated, for reference only, not investment advice*")
    ...
    def _append_market_snapshot(self, lines: List[str], result: AnalysisResult) -> None:
        ...
        lines.extend([
            "### 📈 Market Data",
            "",
            "| Close | Prev | Open | High | Low | Chg% | Chg | Amp | Volume | Turnover |",
            "|------|------|------|------|------|-------|-------|------|--------|--------|",
        ...
        if "price" in snapshot:
            ...
            lines.extend([
                "",
                "| Current | Vol Ratio | Turnover | Source |",
                "|-------|------|--------|----------|",
...
class NotificationBuilder:
    ...
    def build_stock_summary(results: List[AnalysisResult]) -> str:
        ...
        适用于快速通知
        """
        lines = ["📊 **Today's Stock Summary**", ""]
        
        for r in sorted(results, key=lambda x: x.sentiment_score, reverse=True):
            emoji = r.get_emoji()
            advice = normalize_operation_advice(r.operation_advice)
            lines.append(f"{emoji} {r.name}({r.code}): {advice} | Score {r.sentiment_score}")
