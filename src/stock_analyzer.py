...
class TrendStatus(Enum):
    ...
    BULL = "Bull Alignment"          # MA5 > MA10 > MA20
    WEAK_BULL = "Weak Bull"          # MA5 > MA10，但 MA10 < MA20
    CONSOLIDATION = "Consolidation"  # 均线缠绕
    WEAK_BEAR = "Weak Bear"          # MA5 < MA10，但 MA10 > MA20
    BEAR = "Bear Alignment"          # MA5 < MA10 < MA20
    STRONG_BEAR = "Strong Bear"      # MA5 < MA10 < MA20，且间距扩大
...
class VolumeStatus(Enum):
    ...
    SHRINK_VOLUME_UP = "Light Volume Up"       # 无量上涨
    SHRINK_VOLUME_DOWN = "Light Volume Pullback"  # 缩量回调（好）
...
class StockTrendAnalyzer:
    ...
    def _analyze_trend(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if ma5 > ma10 > ma20:
            ...
            if curr_spread > prev_spread and curr_spread > 5:
                ...
                result.ma_alignment = "Strong bull alignment, MAs diverging upward"
                result.trend_strength = 90
            else:
                result.trend_status = TrendStatus.BULL
                result.ma_alignment = "Bull alignment MA5>MA10>MA20"
            ...
            if curr_spread > prev_spread and curr_spread > 5:
                ...
                result.ma_alignment = "Strong bear alignment, MAs diverging downward"
                result.trend_strength = 10
            else:
                result.trend_status = TrendStatus.BEAR
                result.ma_alignment = "Bear alignment MA5<MA10<MA20"
            ...
            result.ma_alignment = "MAs intertwined, trend unclear"
    ...
    def _analyze_volume(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if result.volume_ratio_5d >= self.VOLUME_HEAVY_RATIO:
            if price_change > 0:
                ...
                result.volume_trend = "Heavy volume rally, strong bullish momentum"
            else:
            ...
            if price_change > 0:
                ...
                result.volume_trend = "Light volume rise, weak upward momentum"
            else:
                result.volume_status = VolumeStatus.SHRINK_VOLUME_DOWN
                result.volume_trend = "Light volume pullback, healthy shakeout"
        else:
    ...
    def _analyze_macd(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if is_golden_cross and curr_zero > 0:
            ...
            result.macd_signal = "✅ Golden cross, upward trend"
        elif is_death_cross:
            result.macd_status = MACDStatus.DEATH_CROSS
            result.macd_signal = "❌ Death cross, downward trend"
            ...
            result.macd_signal = "✓ Bullish alignment, sustained uptrend"
        elif result.macd_dif < 0 and result.macd_dea < 0:
            result.macd_status = MACDStatus.BEARISH
            result.macd_signal = "⚠ Bearish alignment, sustained downtrend"
        else:
            result.macd_status = MACDStatus.BULLISH
    ...
    def _analyze_rsi(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        ...
        if rsi_mid > self.RSI_OVERBOUGHT:
            ...
            result.rsi_signal = f"⚠️ RSI overbought ({rsi_mid:.1f}>70), high pullback risk"
        elif rsi_mid > 60:
            ...
            result.rsi_signal = f"⚡ RSI weak ({rsi_mid:.1f}), watch for bounce"
        else:
            result.rsi_status = RSIStatus.OVERSOLD
            result.rsi_signal = f"⭐ RSI oversold ({rsi_mid:.1f}<30), bounce likely"
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        if result.trend_status in [TrendStatus.STRONG_BULL, TrendStatus.BULL]:
            reasons.append(f"✅ {result.trend_status.value}, ride the trend")
        elif result.trend_status in [TrendStatus.BEAR, TrendStatus.STRONG_BEAR]:
            risks.append(f"⚠️ {result.trend_status.value}, avoid long positions")
        ...
        if bias < 0:
            ...
            if bias > -3:
                score += 20
                reasons.append(f"✅ Price slightly below MA5 ({bias:.1f}%), pullback entry")
            elif bias > -5:
                score += 16
                reasons.append(f"✅ Price pulled back to MA5 ({bias:.1f}%), watch support")
            else:
                score += 8
            ...
            score += 14
            reasons.append(f"⚡ Price slightly above MA5 ({bias:.1f}%), small position OK")
        elif bias > effective_threshold:
            score += 4
            risks.append(
                f"❌ Bias too high ({bias:.1f}%>{effective_threshold:.1f}%), do NOT chase!"
            )
        elif bias > base_threshold and is_strong_trend:
            score += 10
            reasons.append(
                f"⚡ Strong trend with elevated bias ({bias:.1f}%), light position OK"
            )
        else:
            score += 4
            risks.append(
                f"❌ Bias too high ({bias:.1f}%>{base_threshold:.1f}%), do NOT chase!"
            )
        ...
        if result.volume_status == VolumeStatus.SHRINK_VOLUME_DOWN:
            reasons.append("✅ Light volume pullback, healthy shakeout")
