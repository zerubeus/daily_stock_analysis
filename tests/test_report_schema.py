...
class TestAnalyzerSchemaFallback(unittest.TestCase):
    ...
    def test_parse_response_valid_json_succeeds(self) -> None:
        """Valid JSON produces correct normalized AnalysisResult."""
        analyzer = GeminiAnalyzer()
        response = json.dumps({
            ...
            "decision_type": "hold",
            "confidence_level": "高",
            "analysis_summary": "Technical setup improving",
        })
        result = analyzer._parse_response(response, "600519", "Stock 600519")
        self.assertIsInstance(result, AnalysisResult)
        ...
        self.assertEqual(result.analysis_summary, "Technical setup improving")
