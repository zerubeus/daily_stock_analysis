    ...
    <div className="rounded-lg border border-white/8 bg-card/40 overflow-hidden">
      ...
      >
        ...
        <span className="text-[11px] text-muted truncate flex-1">
          {modelCount > 0 ? `${modelCount} models` : 'No models configured'}
        </span>

        {/* Status indicators */}
        <span className="flex items-center gap-1.5 shrink-0">
          {testState?.status === 'success' && <span className="h-2 w-2 rounded-full bg-emerald-400" title="Connected" />}
          {testState?.status === 'error' && <span className="h-2 w-2 rounded-full bg-rose-400" title="Connection failed" />}
          {testState?.status === 'loading' && <span className="h-2 w-2 rounded-full bg-amber-400 animate-pulse" title="Testing" />}
          {!hasKey && channel.protocol !== 'ollama' && (
            <span className="text-[10px] text-amber-400/80">No API Key</span>
          )}
        </span>
      ...
      {expanded && (
        <div className="border-t border-white/6 bg-card/20 px-3 py-3 space-y-2.5">
          <div className="grid gap-2 sm:grid-cols-2">
            <div>
              <label className="mb-0.5 block text-[11px] text-muted">Channel name</label>
              <input
            ...
            <div>
              <label className="mb-0.5 block text-[11px] text-muted">Protocol</label>
              <Select
            ...
            <input
              ...
              placeholder={
                channel.protocol === 'gemini' || channel.protocol === 'anthropic'
                  ? 'Official API can be left empty'
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                placeholder={channel.protocol === 'ollama' ? 'Local Ollama can be left empty' : 'Multiple keys comma-separated'}
              />
              <button
                type="button"
          ...
          <div>
            <label className="mb-0.5 block text-[11px] text-muted">Models (comma-separated)</label>
            <input
              className="input-terminal text-sm"
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: 'Channel name cannot be empty and must contain only letters, numbers, or underscores.' });
      return;
    }
    ...
    if (managesRuntimeConfig && availableModels.length > 0) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'Primary model not found in enabled channel models, please re-select.' });
        return;
      }
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: 'Invalid fallback model detected, please re-select.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: 'Vision model not found in enabled channel models, please re-select.' });
        return;
      }
    }
  ...
  return (
    <div className="rounded-xl border border-cyan/20 bg-elevated/50 p-4">
      ...
      >
        <div>
          <h3 className="text-sm font-semibold text-white">AI Model Config</h3>
          <p className="mt-0.5 text-xs text-muted">
            Add provider channels with API keys and model names. Config auto-syncs to .env file.
          </p>
        </div>
        ...
        <div className="mt-4 space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between px-1">
              <span className="text-xs font-medium text-muted uppercase tracking-wider">Channels</span>
              {channels.length > 0 && (
                <span className="text-[10px] text-muted">{channels.filter(c => c.enabled).length}/{channels.length} enabled</span>
              )}
            </div>

          {channels.length === 0 ? (
            <div className="rounded-lg border border-dashed border-white/10 bg-card/20 px-4 py-6 text-center text-xs text-muted">
              No channels yet. Select a provider and click "Add Channel"
            </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-lg border border-white/8 bg-card/30 p-3">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <span className="text-xs font-medium text-muted uppercase tracking-wider">Runtime Parameters</span>
                  <p className="mt-0.5 text-[11px] text-secondary">Auto-inferred when empty</p>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary">
                  Controls output randomness. 0 is deterministic, 2 is max randomness. Recommended: 0.7.
                </p>
              </div>

              {availableModels.length === 0 ? (
                <div className="rounded-lg border border-dashed border-white/10 bg-card/20 px-3 py-2 text-xs text-muted">
                  Add at least one enabled channel with models first. Primary/fallback/Vision options will appear below.
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="mb-2 block text-xs text-muted">Fallback Models</label>
                    ...
                    <p className="mt-1 text-[11px] text-secondary">
                      Fallback models are only used when the primary model fails. The primary model won't be duplicated in fallback.
                    </p>
                  </div>

                  <div>
                    <label className="mb-1 block text-xs text-muted">Vision Model</label>
                    <Select
            ...
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
              LITELLM_CONFIG is currently enabled. Primary/fallback/Vision/Temperature are managed in the general fields below; only channel entries are saved here, without overriding YAML runtime selection.
            </div>
          )}
